    ...
    <section id="achievements" className="bg-ink-light py-28 px-6 md:px-12">
      <div className="max-w-5xl mx-auto">
        <FadeIn className="mb-20">
          <p className="font-sans text-[10px] tracking-[0.45em] uppercase text-gold mb-4">Recognition</p>
          <h2 className="font-serif text-7xl md:text-9xl text-parchment leading-none">Awards</h2>
        </FadeIn>

        <div>
          {achievements.map((item, i) => (
            <FadeIn key={i} delay={i * 0.1}>
              <div className="group py-9 border-t-2 border-ink-border flex flex-col md:flex-row md:items-start md:gap-12 hover:border-gold/25 transition-colors duration-500">
                ...
                <div className="flex-1">
                  <p className="font-sans text-xl font-semibold text-parchment mb-3 leading-snug">
                    {item.label}
                  </p>
                ...
                <div className="hidden md:flex items-center pt-2">
                  <div className="w-6 h-[2px] bg-ink-border group-hover:w-12 group-hover:bg-gold transition-all duration-500" />
                </div>
              </div>
            </FadeIn>
          ))}
          <div className="border-t-2 border-ink-border" />
        </div>
      </div>
    </section>
  )
